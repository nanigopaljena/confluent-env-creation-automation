variable "crn_pattern" {
  description = "Global CRN pattern for Confluent resources"
  type        = string
  default     = "crn://confluent.cloud/organization=${var.confluent_organization_id}/environment=${var.confluent_environment_id}/kafka-cluster=${var.confluent_kafka_cluster_id}"
}
...
resource "confluent_role_binding" "liam_topic_owner" {
  ...
  crn_pattern = "${var.crn_pattern}/topic=${liam_topic_prefix}*"
}
...
resource "confluent_role_binding" "liam_consumer_group_owner" {
  ...
  principal   = "User:${confluent_service_account.liam[0].id}"
  role_name   = "ResourceOwner"
  crn_pattern = "${var.crn_pattern}/consumer-group=${var.liam_project_name}*"
}
...
resource "confluent_role_binding" "gravitee_consumer_group_owner" {
  ...
  role_name   = "ResourceOwner"
  crn_pattern = "${var.crn_pattern}/consumer-group=*"
}
