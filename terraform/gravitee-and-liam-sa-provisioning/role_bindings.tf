...
resource "confluent_role_binding" "liam_consumer_group_owner" {
  ...
  principal   = "User:${confluent_service_account.liam[0].id}"
  role_name   = "ResourceOwner"
  crn_pattern = "crn://confluent.cloud/organization=${var.confluent_organization_id}/environment=${var.confluent_environment_id}/kafka=${var.kafka_cluster_id}/consumer-group=${var.liam_project_name}*"
}
...
resource "confluent_role_binding" "gravitee_consumer_group_owner" {
  ...
  role_name   = "ResourceOwner"
  crn_pattern = "crn://confluent.cloud/organization=${var.confluent_organization_id}/environment=${var.confluent_environment_id}/kafka=${var.kafka_cluster_id}/consumer-group=*"
